...
def _update_run_kernel(opt, clip_value, beta1, beta2, eps, lr, weight_decay,
                       param, m, v, gradient, decay_flags, optim_filter):
    """
    Update parameters by FusedCastAdamWeightDecay op.
    """
    success = True
    if optim_filter:
        if decay_flags:
...
class FusedCastAdamWeightDecay(Optimizer):
    ...
    Examples:
        >>> net = Net()
        >>> #1) All parameters use the same learning rate and weight decay
        >>> optim = FusedCastAdamWeightDecay(params=net.trainable_params())
        >>>
        >>> #2) Use parameter groups and set different values
        ...
        ...                 {'order_params': net.trainable_params()}]
        >>> optim = FusedCastAdamWeightDecay(group_params, learning_rate=0.1, weight_decay=0.0)
