...
def chroma_filter_bank(
    ...
    Args:
        ...
        weighting_parameters (`Tuple[float]`, *optional*, defaults to `(5., 2.)`):
            If specified, apply a Gaussian weighting parameterized by the first element of the tuple being the
            center and the second element being the Gaussian half-width.
...
def spectrogram(
    ...
    if power is None and mel_filters is not None:
        raise ValueError(
            "You have provided `mel_filters` but `power` is `None`. Mel spectrogram computation is not yet supported "
            "for complex-valued spectrogram. Specify `power` to fix this issue."
        )
...
def spectrogram_batch(
    ...
    """
    Calculates spectrograms for a list of waveforms using the Short-Time Fourier Transform, optimized for
    batch processing.
    This function extends the capabilities of the `spectrogram` function to handle multiple waveforms efficiently by
    ...
    How this works:
        ...
        1. The input waveform is split into frames of size `frame_length` that are partially overlapping by
            `frame_length - hop_length` samples.
        2. Each frame is multiplied by the window and placed into a buffer of size `fft_length`.
        3. The DFT is taken of each windowed frame.
        4. The results are stacked into a spectrogram.
    ...
    Note: This function is designed for efficient batch processing of multiple waveforms but retains compatibility
    with individual waveform processing methods like `librosa.stft`.
...
def amplitude_to_db_batch(
    ...
    """
    Converts a batch of amplitude spectrograms to the decibel scale. This
    computes `20 * log10(spectrogram / reference)`, using basic logarithm properties for numerical stability.
