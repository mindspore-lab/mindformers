...
class Bucket:
    ...
    def inplace_reduce_dp(self, src, target):
        ...
        if self.ddp_config.average_in_collective:
            target.copy_(
                mint.div(
                    self.grad_reducer(src, 'sum', self.data_parallel_group),
    ...
    def issue_grad_reduce(self):
        ...
        if self.data_parallel_world_size > 1:
            with hal.StreamCtx(self.comm_stream):
                ...
                self.inplace_reduce_dp(self.grad_data, self.grad_data[start_idx:end_idx])
...
class ParamAndGradBuffer:
    ...
    def __init__(
        ...
        for param in params[::-1]:
            ...
            if _need_new_bucket(last_bucket_numel):
            ...
            data_padded_numel = _pad_param(param.numel())
        ...
        for param in params:
            ...
            param.grad_view = self._get_buffer_slice(param.shape,
                                                     data_start_index,
                                                     BufferType.GRAD)
    ...
    def _get_buffer_slice(self, shape, start_index, buffer_type):
        ...
        if not (start_index >= 0 and end_index <= self.numel):
            raise RuntimeError("when get buffer slice, start index should be greater than 0 and end index "
                               "should be less than buffer size but got {} and {}.".format(start_index, end_index))
        if buffer_type == BufferType.PARAM:
            if self.param_data is None:
                raise RuntimeError("Can not get param data buffer slice when param data is None")
    ...
    def all_finite(self):
        " get finite status of all buffer's grad data. "
    ...
    def final_grad_reduce(self):
        ...
        if self.ddp_config.check_for_nan_in_grad:
            self._all_finite = (mint.isfinite(self.grad_data)).all()
