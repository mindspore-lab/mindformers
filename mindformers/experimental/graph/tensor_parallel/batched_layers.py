...
from mindspore.ops.auto_generate import Cast, BatchMatMulExt, AddExt, Reshape, Transpose
...
class ColumnParallelBatchedLinear(nn.Cell):
    ...
    def __init__(self,
        ...
        if self.has_bias:
            ...
            # add expert dimension
            bias_shape = [config.num_moe_experts, 1] + bias_shape
            self.bias = Parameter(bias_init(bias_shape), name='bias')
        else:
            self.bias = None
    ...
    def shard(self, config: TransformerConfig) -> None:
        ...
        if not self.skip_bias_add:
            add_in_strategy = ((dp * ep, 1, tp), (ep, 1, tp))
...
class RowParallelBatchedLinear(nn.Cell):
    ...
    def shard(self, config: TransformerConfig) -> None:
        ...
        if not self.skip_bias_add:
            add_in_strategy = ((dp * ep, 1, 1), (ep, 1, 1))
            self.add.shard(in_strategy=add_in_strategy)
