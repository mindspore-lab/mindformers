...
class VersionCheck(BaseCheck):
    ...
    def check(self):
        ...
        try:
            ...
            if not Path(cann_info_file).exists(): raise RuntimeError('Cannot find cann info file')
            ...
            if not Path(driver_info_file).exists(): raise RuntimeError('Cannot find driver info file')
