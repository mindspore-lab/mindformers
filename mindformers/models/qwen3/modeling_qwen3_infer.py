...
class InferenceQwen3ForCausalLM(Qwen3PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.config = config
        config = convert_to_transformer_config(self.config)
    ...
    def load_weights(self, load_checkpoint):
        r"""
        Load weights.
        ...
        keys = []
        if sf_files:
            with safe_open(os.path.join(load_checkpoint, sf_files[0]), framework="np") as f:
                keys = f.keys()
        for key in keys:
            ...
            if key.split('.')[-2] == 'linear_qkv':
                ...
                break
        non_layer_weights, layer_weights = (self.convert_hf_weight_to_mf(load_checkpoint))
        mf_hf_weights = {}
        for weight_name in list(non_layer_weights.keys()):
            ...
            mf_hf_weights[new_name] = weight_name
        parameter_dict = self.model.load_weights(load_checkpoint, non_layer_weights, mf_hf_weights, source_qkv_concat)
        ...
        with tqdm(range(num_layers), desc="Weight loading", disable=not enable_tqdm) as pbar:
            for layer_id in pbar:
                ...
                train_prefix = f"model.decoder.layers.{layer_id}."
                ...
                parameter_dict = self.model.load_weights(
                    load_checkpoint, layer_weight, mf_hf_weights, source_qkv_concat)
