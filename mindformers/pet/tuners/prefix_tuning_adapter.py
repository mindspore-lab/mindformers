...
# ============================================================================
"""
Note: prefix_tuning algrithm for mindformers' pretrained model.
Reference: https://arxiv.org/pdf/2101.00190.pdf
"""
...
class PrefixEncoder(nn.Cell):
    ...
    def __init__(
        ...
        self.mindpet_delta_prefixtuning_prefix = ms.Parameter(
        ...
        self.mindpet_delta_prefixtuning_embedding = PrefixEmbedding(vocab_size=self.pre_seq_len,
        ...
        if self.prefix_projection:
            ...
            self.mindpet_delta_prefixtuning_dense_in = nn.Dense(out_embed_dim, self.projection_dim, dtype=ms.float16)
            self.mindpet_delta_prefixtuning_tanh = nn.Tanh()
            self.mindpet_delta_prefixtuning_dense_out = nn.Dense(self.projection_dim, out_embed_dim, dtype=ms.float16)
            self.mindpet_delta_prefixtuning_trans = nn.SequentialCell(
                self.mindpet_delta_prefixtuning_dense_in,
                self.mindpet_delta_prefixtuning_tanh,
                self.mindpet_delta_prefixtuning_dense_out,
            )
    ...
    def construct(self, batch_size, dtype=mstype.half):
        ...
        new prefix
        """
        prefix_tokens = self.expand_dims(self.mindpet_delta_prefixtuning_prefix, 0)
        prefix_tokens = self.tile(prefix_tokens, (batch_size, 1))

        # (bs, pre_len) -> (bs, pre_len, 2 * layers * num_heads * kv_channels)
        past_key_values = self.mindpet_delta_prefixtuning_embedding(prefix_tokens)

        if self.prefix_projection:
            past_key_values = self.mindpet_delta_prefixtuning_trans(past_key_values)
    ...
    def shard(self, data_parallel, model_parallel):
        ...
        if self.prefix_projection:
            ...
            self.mindpet_delta_prefixtuning_dense_in.matmul.shard(((data_parallel, 1), (model_parallel, 1)))
            self.mindpet_delta_prefixtuning_dense_in.bias_add.shard(((data_parallel, 1), (1,)))
            self.mindpet_delta_prefixtuning_tanh.tanh.shard(((data_parallel, 1, 1),))
            self.mindpet_delta_prefixtuning_dense_out.matmul.shard(((data_parallel, 1), (model_parallel, 1)))
            self.mindpet_delta_prefixtuning_dense_out.bias_add.shard(((data_parallel, 1), (1,)))
...
class PrefixTuningAdapter(PetAdapter):
    ...
    """
    @classmethod
    @args_type_check(config=(dict, PrefixTuningConfig))
    def get_pet_model(cls, model: nn.Cell = None, config: Union[dict, PrefixTuningConfig] = None):
        pass

    @classmethod
    @args_type_check(config=(dict, PrefixTuningConfig))
    def get_prefix(cls, model: nn.Cell = None, config: Union[dict, PrefixTuningConfig] = None):
        """
        return prefix prompt.
        """
        if not isinstance(config, PrefixTuningConfig):
            config = config.copy()
            config.pop("pet_type")
        ...
        pre_seq_len = config["prefix_token_num"]

        prefixs = PrefixEncoder(pre_seq_len=pre_seq_len, num_layers=num_layers, num_heads=num_heads,
