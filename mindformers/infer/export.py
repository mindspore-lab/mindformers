...
def get_bloom_inc_model_input(batch_size, seq_length, prefill):
    ...
    return input_ids, input_position, None, None, init_reset, batch_valid_length
...
def get_llama_inc_model_input(batch_size, seq_length, prefill):
    ...
    init_reset = ms.Tensor([init_reset], mstype.bool_)
    batch_valid_length = ms.Tensor([128] * batch_size, mstype.int32)
    return input_ids, None, input_position, None, None, init_reset, batch_valid_length
...
def get_glm_inc_model_input(batch_size, seq_length, prefill):
    ...
    current_index = ms.Tensor([127] * batch_size, mstype.int32)
    init_reset = not prefill
    init_reset = ms.Tensor([init_reset], mstype.bool_)
    batch_valid_length = ms.Tensor([128] * batch_size, mstype.int32)
    return input_ids, position_ids, attention_mask, current_index, init_reset, batch_valid_length
...
def main(args_):
    ...
    if not config.infer.increment_model_path:
        export_single_model(config, args_.batch_size, args_.model_type)
    else:
        export_inc_model(config, args_.batch_size, args_.model_type)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--device_id', default=0,
        type=int,
        help='device id of export mindir.')
