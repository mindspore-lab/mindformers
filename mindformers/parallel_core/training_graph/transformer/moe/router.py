...
class TopKRouter(Router):
    ...
    def _group_limited_topk(self, router_prob):
        ...
        Args:
            router_prob: Gating activation generated by the router.
        ...
        group_scores = self.reshape(router_prob_with_bias,
                                    (router_prob_with_bias.shape[0],
                                     ...
                                     self.num_groups, -1))
        top_m = self.num_experts_chosen // self.group_topk
        top_values, _ = self.tc_topk(group_scores, top_m)
        group_scores = self.tc_sum(top_values, -1)
        _, top_idx_group = self.topk(group_scores, self.group_topk)
        dp, tok_num, topk_group = group_scores.shape
        group_mask = ops.one_hot(top_idx_group, topk_group, on_value=1.0, off_value=0.0)  # (dp, N, topk_group, G)
        group_mask = group_mask.sum(axis=2)  # (dp, N, G)
        score_mask = self.reshape(group_mask, (dp, tok_num, topk_group, 1))
        score_mask = score_mask.repeat_interleave(self.expert_dim // self.num_groups, dim=-1)
        score_mask = self.reshape(score_mask, (dp, tok_num, -1))  # (dp, N, n_routed_experts)
        masked_scores = ops.masked_fill(router_prob_with_bias, ~score_mask.bool(), 0.0)

        # (dp, N, k) fp32,  (dp, N, k) int32 <-- (dp, N, n_routed_experts)
        if self.moe_router_enable_expert_bias:
            _, expert_index = self.topk(masked_scores, self.num_experts_chosen)
            ...
            expert_gate, expert_index = self.topk(masked_scores, self.num_experts_chosen)
            expert_index = self.cast(expert_index, mstype.int32)
    ...
    def shard(self, config: TransformerConfig):
        ...
        if self.group_topk:
            ...
            self.tc_sum.add_prim_attr("self_define_shard", True)
            self.tc_sum.shard(in_strategy=(layout(dp, "None", "None", "None"),),
                              out_strategy=(layout(dp, "None", "None"),))
