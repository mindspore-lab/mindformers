...
class LayerSetting:
    ...
    def __init__(self, num_layers, offset, config, pp_interleave_num=1, start_stage=0, stage_num=0):
        ...
        self.select_recompute = config.select_recompute
        self.select_comm_recompute = config.select_comm_recompute
        self.recompute_slice_activation = config.recompute_slice_activation

        self.gradient_aggregation_group = config.gradient_aggregation_group
        ...
        self.layer_recompute = self._format_recompute_list(self.recompute)
        self.select_recompute = self._format_recompute_dict(
            self.select_recompute, default_patterns)
        self.select_comm_recompute = self._format_recompute_dict(
            self.select_comm_recompute, default_comm_patterns)
    ...
    def set(self, layer, layer_id):
        ...
        if self.recompute:
            if isinstance(self.recompute, bool):
                if self.recompute:
                    layer.recompute(recompute_slice_activation=self.recompute_slice_activation)
