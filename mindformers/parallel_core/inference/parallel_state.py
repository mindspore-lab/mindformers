...
def initialize_moe_model_parallel(expert_model_parallel_size: int,
    ...
    for local_ranks in moe_rank_generator.get_ranks('tp'):
        global_ranks = [
            tensor_and_data_parallel_group.global_ranks[r]
            for r in local_ranks
            if r < len(tensor_and_data_parallel_group.global_ranks)
        ]
...
def initialize_model_parallel(tensor_model_parallel_size: int = 1,
    ...
    tensor_and_data_parallel_group = create_process_group('tp-dp', group_name_prefix='tp_dp')
