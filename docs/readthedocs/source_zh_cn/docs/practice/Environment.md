...
|PLOG_PATH_CHANGE|控制plog日志是否改变存储路径|bool|True:存储到./ouput目录下 <br> False: 默认存储位置 <br> 无默认值|设置之后方便用户查询plog日志|
|MF_LOG_SUFFIX|设置所有log日志文件夹的自定义后缀|String|log文件夹的后缀 <br> 默认值：当前的时间戳|添加一致的后缀，可以隔离各个任务的日志，不会被覆写|
