...
.. py:class:: mindformers.AutoModelForZeroShotImageClassification
    ...
    这是一个通用的模型类，使用类方法 `AutoModelForZeroShotImageClassification.from_pretrained` 或 `AutoModelForZeroShotImageClassification.from_config` 创建时会自动实例化为库中（带有零样本图片分类建模头）的模型类之一。
    这个类不能直接使用 \_\_init\_\_() 实例化（会抛出异常）。

    .. py:method:: from_config(config, **kwargs)
        :classmethod:

        通过Config实例或者yaml文件实例化一个库中（带有零样本图片分类建模头）的模型类。
