...
用户只需提供必要的数据、配置文件、权重等信息，执行[pretrain_tool.sh](https://gitee.com/mindspore/mindformers/blob/r1.5.0/scripts/benchmark/pretrain_tool.sh)脚本即可启动相应训练、微调任务。其中数据支持原始数据、预处理之后的数据，部分数据支持在线模式，即只提供数据集名称即可。脚本参数介绍如下：
