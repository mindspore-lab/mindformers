...
from xformer.tools.register import XFormerConfig, ActionDict
from xformer.common.parallel_config import build_parallel_config
from xformer.tools.utils import str2bool
from xformer.common.context import build_context
from xformer.trainer import build_trainer
from xformer.common.callback import build_callback
from xformer.tools.cloud_adapter import cloud_monitor
...
def main(config):
    ...
    if config.pretrain_dataset:
        config.pretrain_dataset.data_loader.dataset_dir = cfts.get_dataset(config.pretrain_dataset.data_loader.dataset_dir)
    if config.eval_dataset:
        config.eval_dataset.data_loader.dataset_dir = cfts.get_dataset(config.eval_dataset.data_loader.dataset_dir)
...
if __name__ == "__main__":
    ...
    config_ = XFormerConfig(args_.config)
