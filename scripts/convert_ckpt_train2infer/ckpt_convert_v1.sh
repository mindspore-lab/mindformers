...
if [ "$train_to_infer" == "true" ] && [ "$precision" == "fp16" ]; then
    ...
    if [ -z "$src_ckpt_path" ] || [ -z "$yaml_path" ] ; then
        echo "Please set fp16 checkpoint saved path and w8a16 yaml file"
        exit 1
    fi
    if [ -z "$dst_ckpt_path" ]; then
    ...
    if [ "$dir_count" != "$world_size" ] ; then
        if [[ -e "${Dst_ckpt_path}_${dir_count}p/rank_0/*.ckpt" ]]; then
        ...
        if [[ -e "${Infer_strategy_path}_${dir_count}p/strategy/ckpt_strategy_rank_0.ckpt" ]] ; then
            ...
            --log_dir=./log/msrun_log_save_strategy_${precision}_${dir_count}p --master_port=8126 --join=True --bind_core=False \
            python save_strategy.py \
            --yaml_file=$yaml_path \
        ...
        if [[ -e "${Infer_strategy_path}_${world_size}p/strategy/ckpt_strategy_rank_0.ckpt" ]] ; then
            echo "Has fp16_${world_size}p strategy, jump to step 4"
        else
    ...
    # 检查输入值
    if [ -z "$src_ckpt_path" ] || [ -z "$yaml_path" ] ; then
        echo "Please set fp16 checkpoint saved path and w8a8 yaml file"
        exit 1
    fi
    if [ -z "$dst_ckpt_path" ]; then
       Dst_ckpt_path=./infer_ckpt/${precision}
