...
if [ "$train_to_infer" == "true" ] && [ "$precision" == "fp16" ]; then
    ...
    if find "${Dst_ckpt_path}_${rank_num}p/rank_0/" -type f -name "*.ckpt" | read; then
        echo "Has ${precision}_${rank_num}p ckpt, jump to step 2"
    else
        if [ "$precision" == "w8a16" ]; then
            #1. 转换成rank_num的w8a16权重
            echo "-----1. Start to convert ${rank_num}p ${precision} weights time: $(date +%H:%M:%S) -----"
            ...
            echo "-----Start to convert ${rank_num}p ${precision} to ${world_size}p ${precision} -----"
