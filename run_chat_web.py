...
def predict(inputs, bot, history, do_sample, top_k, top_p, repetition_penalty, max_length, prompt):
    """predict"""
    if inputs == "":
    ...
    if prompt == "":
        prompted_input = inputs
    elif prompt.find("{}") != -1:
        prompted_input = prompt.format(inputs)
    else:
    ...
    input_tokens = tokenizer(prompted_input)
    ...
    if max_length <= len(input_tokens["input_ids"]):
        raise gr.Error("Max length must be set larger than the length of input tokens! The current length of input "
    ...
    logger.info("Start to generate text. Generate args: {input_ids: %s, do_sample: %s, top_k: %s, top_p: %s, "
                "repetition_penalty: %s, max_length: %s}",
                input_tokens["input_ids"], do_sample, top_k, top_p, repetition_penalty, max_length)
    thread.start()
    output = ""
...
def set_do_sample_args(do_sample):
    if do_sample:
        return {top_k_slider: gr.update(visible=True), top_p_slider: gr.update(visible=True)}
    return {top_k_slider: gr.update(visible=False), top_p_slider: gr.update(visible=False)}
...
with gr.Blocks() as demo:
        ...
        with gr.Column(scale=2):
            with gr.Group():
                ...
                top_k_slider = gr.Slider(value=config.top_k if config.top_k else 1,
                                         label="top k", maximum=10, minimum=1, step=1,
    ...
    submit_btn.click(predict,
                     [user_input, chatbot, chat_history, do_sample_checkbox, top_k_slider, top_p_slider, rp_number,
                      max_len_number, prompt_input],
    ...
    do_sample_checkbox.change(set_do_sample_args, [do_sample_checkbox], [top_k_slider, top_p_slider])
