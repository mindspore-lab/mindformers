...
def attn_rearange(pt_param: Dict, config: YiZhaoConfig):
    """attn_rearange"""
    ...
    for layer, params in qkv_bias.items():
        ...
        bq = rearange_b(bq, head_dim)
        bk = rearange_b(bk, head_dim)
        bv = rearange_b(bv, head_dim)
        concat_qkv = np.concatenate(
            [bq, bk, bv],
            axis=0
        )
...
def attn_merge(pt_param: Dict, config: YiZhaoConfig):
    """attn_merge"""
    kv_channels = config.kv_channels
    head_dim = kv_channels
    ...
    for layer, params in qkv_weights.items():
        if config.use_llama_rope:
            params["wq"]['param'] = rearange_w(params["wq"]['param'], head_dim)
            params["wk"]['param'] = rearange_w(params["wk"]['param'], head_dim)
    ...
    for layer, params in qkv_bias.items():
        if config.use_llama_rope:
            params["wq"]['param'] = rearange_b(params["wq"]['param'], head_dim)
            params["wk"]['param'] = rearange_b(params["wk"]['param'], head_dim)
        merged_name = params['wq']['name'].replace("wq", "query_key_value")
        merged_param = np.concatenate(
            [
                params["wq"]['param'],
...
def mlp_merge(pt_param: Dict, config: YiZhaoConfig):
    """mlp_merge"""
...
# pylint: disable=W0613
def convert_ms_to_pt(input_path, output_path, config, dtype=pt.float32, **kwargs):
...
if __name__ == '__main__':
    ...
    convert_ms_to_pt(opt.mindspore_ckpt_path, opt.torch_ckpt_path, config=opt.config, dtype=mapping[opt.dtype])
