...
#     MoEParallelConfig
from op_parallel_config import default_dpmp_config, _PipeLineConfig, OpParallelConfig, _Config,  \
    MoEParallelConfig
...
class MultiHeadAttention(Cell):
    ...
    def _attn(self, query, key, value, attention_mask):
        ...
        core_a = query.gather(Tensor(np.random.randint(0, 511, 20)), 2)
        core_a_t = core_a.transpose(*(0, 1, 3, 2))
        core_s1 = self.batch_matmul(query, core_a_t)
        core_s2 = self.batch_matmul(core_a, key)
        s2_ori_dtype = P.DType()(core_s2)
        s1_ori_dtype = P.DType()(core_s1)
        core_s2 = P.Cast()(core_s2, self.softmax_dtype)
        core_s1 = P.Cast()(core_s1, self.softmax_dtype)
        ...
        s2_attention_probs = self._softmax(core_s2)
        s2_attention_probs = P.Cast()(s2_attention_probs, s2_ori_dtype)
        s2_attention_probs = self.prob_dropout(s2_attention_probs)

        s1_attention_probs = self._softmax(core_s1)
        s1_attention_probs = P.Cast()(s1_attention_probs, s1_ori_dtype)
        s1_attention_probs = self.prob_dropout(s1_attention_probs)

        s2_weighted_values = self.batch_matmul(s2_attention_probs, value)
        s1_weighted_values = self.batch_matmul(s1_attention_probs, s2_weighted_values)
        attention_merge = self._merge_heads(s1_weighted_values)
