...
from research.FastFFN.transformer.layers import _LayerNorm, _Linear
from research.FastFFN.transformer.op_parallel_config import default_dpmp_config, _PipeLineConfig, OpParallelConfig, _Config
...
class FeedForward(Cell):
    r"""
        ...
        Examples:
            ...
            (2, 20, 15)
    """
    def __init__(self, hidden_size,      #修改的地方
                 ffn_hidden_size,
                 dropout_rate,
                 hidden_act='gelu',
                 expert_num=1,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation():    #分布式运行
            ...
            self.fastffnmapping1 = _Linear(in_channels=input_size,
                                           out_channels=8,
                                           activation=None,
                                           has_bias=False,
                                           transpose_b=False,
            ...
            self.fastffnmapping2 = _Linear(in_channels=8,
            ...
            self.fastffnprojection1 = _Linear(in_channels=output_size,
                                              out_channels=8,
                                              transpose_b=False,
                                              has_bias=False,
                                              expert_num=expert_num,
            ...
            self.fastffnprojection2 = _Linear(in_channels=8,
            ...
            if expert_num > 1:
                self.fastffnprojection.shard(strategy_matmul=((dp, ep, 1, mp), (ep, mp, 1)))
            else:
                self.fastffnprojection.shard(strategy_matmul=((dp, mp), (mp, 1)))
            self.fastffnprojection.bias.parallel_optimizer = False
            ...
            if expert_num > 1:
                self.fastffnmapping2.shard(strategy_matmul=((dp, ep, 1, 1), (ep, 1, mp)),
                                           strategy_bias=((dp, ep, 1, mp), (1, ep, 1, mp)),
                                           strategy_activation=((dp, ep, 1, mp),))
            else:
                self.fastffnmapping2.shard(strategy_matmul=((dp, 1), (1, mp)),
            ...
            if expert_num > 1:
                self.fastffnprojection2.shard(strategy_matmul=((dp, ep, 1, mp), (ep, mp, 1)),
                                              strategy_bias=((dp, ep, 1, 1), (1, ep, 1, 1)))
            else:
                self.fastffnprojection2.shard(strategy_matmul=((dp, mp), (mp, 1)),
                                              strategy_bias=((dp, 1), (1,)))
            self.fastffnprojection2.bias.parallel_optimizer = False
            self.dropout = nn.Dropout(1 - dropout_rate)
    ...
    def construct(self, x):
        ...
        hidden = self.fastffnmapping1(x)
        hidden = self.fastffnmapping2(hidden)

        output = self.fastffnprojection1(hidden)
        output = self.fastffnprojection2(output)
...
class CoreTransformer(Cell):
    ...
    def __init__(self,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation():
            ...
            self.aux_loss = Tensor(0.0, mstype.float32)
            if encoder_layers > 0:
                self.encoder = TransformerEncoder(num_layers=encoder_layers,   #2368行
                                                  batch_size=batch_size,
                                                  hidden_size=hidden_size,
                                                  ffn_hidden_size=ffn_hidden_size,
                                                  num_heads=num_heads,
