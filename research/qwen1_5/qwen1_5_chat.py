...
# ============================================================================
"""Helper functions to make 'chat' feature work on Mindformers' Qwen1.5 port."""
...
def make_context(
    ...
    if chat_format == 'chatml':
        ...
        for turn_query, turn_response in reversed(history):
            ...
            total_size = len(system_tokens) + \
                len(turn_tokens) + len(history_tokens)
        ...
        prompt_text = f"{im_start}{system_text}{im_end}\n" + history_text + \
                      f"\n{im_start}user\n{query}{im_end}\n{im_start}assistant\n"
...
def _decode_default(
    ...
    """decode default"""
    trim_decode_tokens = tokenizer.decode(tokens, errors=errors)[raw_text_len - 1:]
    if verbose:
...
def _decode_chatml(
    ...
    trim_decode_tokens = tokenizer.decode(
        tokens[:eod_token_idx], errors=errors)[raw_text_len - 1:]
    if verbose:
        print("\nRaw Generate w/o EOD:",
              tokenizer.decode(tokens, errors=errors)[raw_text_len - 1:])
        print("\nRaw Generate:", trim_decode_tokens)
        print("\nEnd Reason:", end_reason)
    for stop_word in stop_words:
        trim_decode_tokens = trim_decode_tokens.replace(stop_word, "").strip()
...
def chat(
    ...
    max_new_tokens = kwargs.get('max_new_tokens', model_config.seq_length // 4)
    max_window_size = kwargs.get('max_window_size',
                                 model_config.seq_length - max_new_tokens - 48)
