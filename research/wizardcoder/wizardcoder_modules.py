...
from mindformers.modules.transformer import TransformerEncoderLayer, MultiHeadAttention, AttentionMask, \
...
class WizardCoderAttentionMask(AttentionMask):
    r"""
            This is an implementation of wizardcoder attention mask.
            Supported Platforms:
                ``Ascend``
        """
    def __init__(self, seq_length, parallel_config=default_dpmp_config):
        super(WizardCoderAttentionMask, self).__init__(seq_length, parallel_config)
        self.cast_rec_ = P.Cast()

    def construct(self, input_mask):
        """Forward process of the WizardCoder AttentionMask"""
        _check_input_dtype(F.dtype(input_mask), "input_mask", [mstype.float32, mstype.float16], self.cls_name)
        input_mask = self.cast_rec_(self.not_equal(input_mask, 0), mstype.float16)
...
class MultiQueryAttention(MultiHeadAttention):
    ...
    def __init__(self, batch_size,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,):
            if use_seq_parallel:
                ...
                print("Ebaling matml recompuation when seq parallel enabled")
            ...
            if use_seq_parallel:
                ...
                print("Enabling matml recompuation when seq parallel enabled")
                self.projection.matmul.add_prim_attr("recompute", True)
                self.projection.matmul.add_prim_attr("recompute_comm_op", True)
            ...
            self.transpose_one_head = P.Transpose().shard(
                ((parallel_config.data_parallel, 1, 1, 1),))
            self.tile_for_batch_matmul = P.Tile().shard(
                ((parallel_config.data_parallel, parallel_config.model_parallel, 1, 1),))
            self.real_div_one_head = P.RealDiv().shard(((parallel_config.data_parallel, 1, 1, 1), ()))
            # Query
        ...
        print("dp_num = {}, mp = {}".format(parallel_config.data_parallel, parallel_config.model_parallel))
        print("self.flash_attention_flag = ", self.flash_attention_flag)
    ...
    def construct(self, query_tensor, key_tensor, value_tensor, attention_mask, key_past=None,
        ...
        if attention_mask is not None and self.flash_attention_flag is False and len(F.shape(attention_mask)) == 3:
            attention_mask = self.expand_dims(attention_mask, 1)
        if attention_mask is not None and self.flash_attention_flag is True and len(F.shape(attention_mask)) == 4:
        ...
        # print("query.shape = ", F.shape(query))
        # print("attention_mask.dtype = ", F.dtype(attention_mask))
        if self.flash_attention_flag:
            # attention_mask = P.Cast()(attention_mask, self.dtype)
    ...
    def _attn(self, query, key, value, attention_mask):
        ...
        if attention_mask is not None:
            ...
            multiplu_out = self.sub(
                ...
                self.cast_rec(attention_mask, P.DType()(attention_scores)))
...
class WizardCoderTransformerDecoderLayer(TransformerEncoderLayer):
    ...
    def __init__(self,
        ...
        if self.use_past:
            ...
            print("TransformerEncoderLayer shape: ", self.key_shape, self.value_shape)
    ...
    def construct(self, x, input_mask=None, init_reset=True, batch_valid_length=None):
        ...
        # assert np.allclose(input_x.asnumpy()[0][0], 0.2158, 0.001, 0.001)
