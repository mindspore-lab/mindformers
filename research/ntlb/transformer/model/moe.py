...
class MoEConfig:
    ...
    def __init__(self, expert_num=1, capacity_factor=1.1, aux_loss_factor=0.05,
                 num_experts_chosen=1, expert_group_size=None, routing_policy=None, routing_stage=None):
        ...
        # new
        self.routing_policy = routing_policy # None
...
class MoE(Cell):
    ...
    def __init__(self, hidden_size,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation():
            ...
            self.mul = P.Mul()
            self.router = Router(d_model=hidden_size, moe_config=moe_config, routing_policy=moe_config.routing_policy,
                                 routing_stage=moe_config.routing_stage, training=True, parallel_config=parallel_config)
            self.cast = P.Cast()
        else:
            self.hidden_size = hidden_size
            self.expert_dim = moe_config.expert_num
...
class Router(Cell):
    ...
    def __init__(self,
        ...
        if self.routing_policy is None or self.routing_policy == 'topk':
            self.router = TopkRouter(d_model=d_model, moe_config=moe_config, training=training,
                                     parallel_config=parallel_config)
        elif self.routing_policy == 'ntlb':
            self.router = NtlbRouter(d_model=d_model, moe_config=moe_config, training=training,
                                     parallel_config=parallel_config)
        elif self.routing_policy == 'stable':
            self.router = StableRouter(d_model=d_model, moe_config=moe_config, training=training,
                                     parallel_config=parallel_config, routing_stage=routing_stage)
...
class NtlbRouter(TopkRouter):
    r"""
        A router implementation which iteratively routes tokens to experts and make sure no tokens left behind.
        Args:
            d_model (int): The hidden size of each token.
            moe_config(MoEConfig): The configuration of MoE (Mixture of Expert).
            training (bool): The value indicating whether is in training phase.
    ...
    def __init__(self,
                 ...
                 training=True,
                 parallel_config=None):
        super(NtlbRouter, self).__init__(d_model, moe_config, training=True, parallel_config=None)
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation():
            dp = parallel_config.data_parallel
            self.sub2 = P.Sub().shard(((), (dp, 1)))
            self.tile = P.Tile()
            self.expand3 = P.ExpandDims()
        else:
            dp = parallel_config.data_parallel
            self.sub2 = P.Sub().shard(((), (dp, 1)))
            self.tile = P.Tile()
    ...
    def _move_overflowed_tokens_to_second_expert(self, expert_mask, expert_capacity, expert_gate, last_num, expert_chosen_index, cum_tokens):
        """
        Keeping only the tokens that fit within expert_capacity 
        and 
        move the unrouted tokens to the second largest expert.

        last_name: mask_count
        """
        ...
        # position_in_expert's shape: (dp_group, tokens_per_group, self.expert_dim)
        position_in_expert = self.mul4(cumsum, expert_mask)
        # cumulative_tokens_in_expert = position_in_expert
        less_result = self.less(position_in_expert, expert_capacity)
        # expert_mask's shape: (dp_group, tokens_per_group, self.expert_dim)
        expert_mask = self.mul5(less_result, expert_mask)
        # expert_mask_flat's shape: (dp_group, tokens_per_group)
    ...
    def construct(self, router_logits):
        ...
        accum_combine_tensor = 0
        cum_tokens = 0
        tokens_left_to_route = 1
        tokens_left_reshaped = 1
        ...
        for expert_chosen_index in range(self.num_experts_chosen):
            ...
            tokens_left_to_route = self.sub2(tokens_left_to_route, self.reduce_sum2(expert_mask, -1))
            tokens_left_reshaped = self.expand3(tokens_left_to_route, 2)
            tokens_left_reshaped = self.tile(tokens_left_reshaped, (1, 1, 10))
            expert_mask = self.mul10(expert_mask, tokens_left_reshaped)
            ...
            output = self._move_overflowed_tokens_to_second_expert(expert_mask, expert_capacity, expert_gate,
                                                     mask_count, expert_chosen_index, cum_tokens)
            expert_mask, expert_gate, expert_mask_flat, position_in_expert= output[0], output[1], output[2], output[3]
            accum_expert_mask = self.add(accum_expert_mask, expert_mask)
            accum_expert_gate = self.add3(accum_expert_gate, expert_gate)
            mask_count = self.add(mask_count, self.reduce_sum_keep(expert_mask, 1))
            ...
            combine_tensor = self.mul7(expert_gate, expert_mask_flat)
            # combine_tensor's shape: (dp_group, tokens_per_group, self.expert_dim)
            # print('combine_tensor1:', combine_tensor)
            combine_tensor = self.mul8(self.expand(combine_tensor, -1),
                                       self.onehot2(expert_index, self.expert_dim, self.on_value, self.off_value))
            # combine_tensor's shape: (dp_group, tokens_per_group, self.expert_dim, self.expert_capacity)
            combine_tensor = self.mul9(self.expand2(combine_tensor, -1),
...
class StableRouter(TopkRouter):
    ...
    def __init__(self,
                 ...
                 parallel_config=None,
                 routing_stage='s1'):
        super(StableRouter, self).__init__(d_model, moe_config, training=True, parallel_config=None, routing_stage='s1')
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation():
            self.routing_stage = routing_stage
            self.distill_routing_dim = 50
            from mindspore.common.initializer import initializer, Orthogonal 
            self.distill_expert_centroids = initializer(Orthogonal(gain=0.1), [moe_config.expert_num, self.distill_routing_dim], mstype.float32)
            dp = parallel_config.data_parallel
            self.transpose_2d = P.Transpose().shard(((dp, 1),))
            self.cross_entropy = P.SparseSoftmaxCrossEntropyWithLogits()
            self.max = P.Max()
    ...
    def _distilled_loss(self, routing_features, distill_expert_centroids):
        """
        distilled loss for the stable moe
        
        """
        if self.routing_stage == 's1' and self.training:
            transposed_distill_expert_centroids = self.transpose_2d(distill_expert_centroids, (0, 1))
            distill_token_expert_affinities = self.mul11(routing_features, transposed_distill_expert_centroids)
            distill_target, _ = self.max(distill_token_expert_affinities, axis=1)  # len
            distill_loss = self.cross_entropy(distill_token_expert_affinities, distill_target)
        else:
            distill_loss = 0
        print(distill_loss)
        return distill_loss
    ...
    def construct(self, router_logits):
        ...
        for expert_chosen_index in range(self.num_experts_chosen):
            ...
            if self.routing_stage == 's1':
                # the balance loss is computed at each routing step
                loss = self.add_scala(loss, self._auxiliary_loss(expert_mask, router_prob_normal))
                # the distill loss is also computed at each routing step
                # loss = self.add_scala(loss, self._distilled_loss(router_logits, self.distill_expert_centroids))
