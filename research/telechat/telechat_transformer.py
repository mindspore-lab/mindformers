...
class TelechatAttention(nn.Cell):
    ...
    def __init__(self,
        ...
        if self.model_name == "telechat_52b":
            self.w_qkv = TelechatLinear(in_channels=self.hidden_size,
    ...
    def construct(self, x: Tensor, freqs_cis: Tuple[Tensor, Tensor], mask=None, batch_valid_length=None,
        ...
        if self.model_name == "telechat_52b":
            query_key_value = self.cast(self.w_qkv(x), self.dtype)
            query, key, value = self.split_qkv(query_key_value, (self.hidden_size, self.kv_dim, self.kv_dim), 2)
        else:
...
class TelechatDecodeLayer(nn.Cell):
    ...
    def __init__(self,
        ...
        self.attention = TelechatAttention(model_name=model_name,
                                           dim=dim,
                                           n_heads=n_heads,
        ...
        self.feed_forward = TelechatFeedForward(model_name=model_name,
