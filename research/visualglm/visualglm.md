...
1. DEVICE_ID：NPU卡的编号，默认是0
2. SEQ_LENGTH：token长度，默认是32，建议设置512
3. BATCH_SIZE：批量推理的批次大小，默认是1

**注意**

1. 设置use_past=True后会生成两个mindir的模型，分别在output/mindir_full_checkpoint和output/mindir_inc_checkpoint目录下。
2. 反之不设置use_past或者use_past=False，则只生成mindir_full_checkpoint一个目录，且后续无法使用增量推理。
...
1. FULL_MODEL_PATH：上面生成的全量模型的路径，比如：output/mindir_full_checkpoint_bs_1_512/rank_0_graph.mindir，这里中间路径会带上
 batch_size和seq_length的后缀，比如batch_size为1，seq_length为512下路径为output/mindir_full_checkpoint_bs_1_512/rank_0_graph.mindir
2. INC_MODEL_PATH：上面生成的全量模型的路径，比如：output/mindir_inc_checkpoint_bs_1_512/rank_0_graph.mindir，这里中间路径会带上
  batch_size和seq_length的后缀，比如batch_size为1，seq_length为512下路径为output/mindir_inc_checkpoint_bs_1_512/rank_0_graph.mindir
3. DEVICE_ID：NPU卡的编号，默认是0
4. SEQ_LENGTH：token长度，默认是32，建议设置512
5. IMAGE_PATH：推理的图片路径，比如examples/titanic.jpg
6. PROMPT：提示词，要加上引号

**注意**

1. 推理设置的batch_size和seq_length要跟上面导出命令的batch_size和seq_length保持一致，否则会出错
