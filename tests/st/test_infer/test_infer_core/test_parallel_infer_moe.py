...
def test_parallel_infer_moe():
    """
    Feature: MoE of Parallel for prediction.
    Description: Test parallel MoE for prediction.
    ...
    commands = [
        (f"export ASCEND_RT_VISIBLE_DEVICES=0,1 && "
         f"msrun --worker_num=2 --local_worker_num=2 --master_port=8110 --log_dir=test_log_moe_graph --join=True "
         f"{cur_dir}/run_parallel_infer_moe.py --mode 0", 'test_log_moe_graph/worker_0.log'),
        (f"export ASCEND_RT_VISIBLE_DEVICES=2,3 && "
         f"msrun --worker_num=2 --local_worker_num=2 --master_port=8112 --log_dir=test_log_moe_pynative --join=True "
         f"{cur_dir}/run_parallel_infer_moe.py --mode 1", 'test_log_moe_pynative/worker_0.log'),
    ]
