...
class TestParallelTransformer:
    ...
    def test_transformer_golden(self):
        ...
        cmd = f"msrun --worker_num={device_num} "+\
                    ...
                    f"--master_port=8118 "+\
                    f"--log_dir=msrun_log_graph_transformer "+\
                    f"--join=True "+\
    ...
    def test_transformer_pynative(self):
        ...
        cmd = f"msrun --worker_num={device_num} "+\
                    f"--local_worker_num={device_num} "+\
                    f"--master_port=8118 "+\
                    f"--log_dir=msrun_log_pynative_transformer "+\
                    f"--join=True "+\
                    f"--cluster_time_out=300 "+\
                    f"{scripts_cmd}"
        ret = os.system(cmd)
    ...
    def test_pynative_with_golden_loss(self):
        ...
        """
        golden_log_path = f'msrun_log_graph_transformer/worker_0.log'
        pynative_log_path = f'msrun_log_pynative_transformer/worker_0.log'
        ...
        assert os.path.exists(golden_log_path) and os.path.exists(pynative_log_path), \
            f"{golden_log_path} or {pynative_log_path} did not exits, " + \
                "please run test_transformer.py to generate them by running below command: \n" + \
                "`pytest -sv test_transformer.py::TestParallelTransformer`"
