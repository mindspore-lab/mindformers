...
class LlamaTransformerLayerNet(nn.Cell):
    """ LlamaTransformerLayerNet. """
    def __init__(
            self,
            seq_length,
            hidden_size,
        ...
        super(LlamaTransformerLayerNet, self).__init__()
        self.freqs_mgr = FreqsMgr(head_dim=hidden_size//num_attention_heads,
                                  seq_length=seq_length,
                                  rotary_dtype=mstype.float32)
        self.layer = LLamaDecodeLayer(seq_length=seq_length,
                                      layer_id=1,
                                      dim=hidden_size,
    ...
    def construct(self, x, attention_mask, labels):
        ...
        output = self.layer(x, freqs_cis, attention_mask)
...
def generate_golden():
    ...
    network = LlamaTransformerLayerNet(seq_length=seq_length,
...
def run_parallel_transformer_layer():
    ...
    config = TransformerConfig(vocab_size=1,
                               ...
                               norm_epsilon=1.e-5)
    network = ParallelTransformerLayerNet(config=config, with_rope=True)
    save_golden = False
    if save_golden:
        golden_ckpt_path = "transformerlayer_golden.ckpt"
        assert os.path.exists(golden_ckpt_path), \
            ...
            "`pytest -sv test_transformerlayer.py::TestParallelTransformerLayer::generate_golden`"
    ...
    golden_losses = [319.64258, 114.767044, 327.66397]
