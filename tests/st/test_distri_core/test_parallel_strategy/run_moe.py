...
from mindformers.experimental.parallel_core.pynative.transformer.moe import MoELayer
...
from mindformers.experimental.parallel_core.pynative.transformer import Module
from mindformers.experimental.parallel_core.pynative.dist_checkpointing import load_checkpoint, save_checkpoint

from tests.st.test_distri_core.utils import transform_moe_golden_params_to_pynative_params, train, _transform_ckpt_helper
...
def run_moe_pynative(model_config, args):
    ...
    if dp == 2: # generate src ckpt
        save_checkpoint(model_config, network, optimizer, None, f"./output/transformer_0")
    else:
        _transform_ckpt_helper(model_config, network, optimizer, f"./output/transformer_0", f"./output/transformer_0_1")
        load_checkpoint(model_config, network, optimizer, None, f"./output/transformer_0_1")
