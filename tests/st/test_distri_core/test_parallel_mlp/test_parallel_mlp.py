...
class TestParallelMLP:
    ...
    def test_mlp_golden(self):
        """
        Feature: generate mlp golden
        Description: run graph mode mlp to generate golden ckpt and loss
        ...
        cmd = f"msrun --worker_num={device_num} "+\
                    ...
                    f"--master_port=8118 "+\
                    f"--log_dir=msrun_log_graph_mlp "+\
                    f"--join=True "+\
                    f"--cluster_time_out=300 "+\
                    f"{scripts_cmd}"
        ret = os.system(cmd)
        os.system(f"grep -E 'ERROR|error' {sh_path}/msrun_log_graph_mlp/worker_0.log -C 3")
        assert ret == 0, "msrun failed, please check msrun_log_graph_mlp/worker_*.log"

    @pytest.mark.level1
    @pytest.mark.platform_arm_ascend910b_training
    @pytest.mark.env_single
    @pytest.mark.run(order=2)
    def test_mlp_pynative(self):
        """
        ...
        cmd = f"msrun --worker_num={device_num} "+\
                    f"--local_worker_num={device_num} "+\
                    f"--master_port=8118 "+\
                    f"--log_dir=msrun_log_pynative_mlp "+\
                    f"--join=True "+\
                    f"--cluster_time_out=300 "+\
                    f"{scripts_cmd}"
        ret = os.system(cmd)
        os.system(f"grep -E 'ERROR|error' {sh_path}/msrun_log_pynative_mlp/worker_0.log -C 3")
        assert ret == 0, "msrun failed, please check msrun_log_pynative_mlp/worker_*.log"
    ...
    def test_pynative_with_golden_loss(self):
        ...
        """
        golden_log_path = f'msrun_log_graph_mlp/worker_0.log'
        pynative_log_path = f'msrun_log_pynative_mlp/worker_0.log'
...
class TestParallelMLPHasGate:
    """A test class for testing MLP."""
    # os.environ['ASCEND_RT_VISIBLE_DEVICES']="1，6"
    @pytest.mark.level1
    @pytest.mark.platform_arm_ascend910b_training
    @pytest.mark.env_single
    @pytest.mark.run(order=1)
    def test_mlp_golden(self):
        ...
        """
        os.environ['GRAPH_OP_RUN'] = "1"
        scripts_name = "run_parallel_mlp_has_gate.py"
        device_num = 1
        ...
        cmd = f"msrun --worker_num={device_num} "+\
                    f"--local_worker_num={device_num} "+\
                    f"--master_port=8118 "+\
                    f"--log_dir=msrun_log_graph_mlp_has_gate "+\
                    f"--join=True "+\
                    f"--cluster_time_out=300 "+\
                    f"{scripts_cmd}"
        ret = os.system(cmd)
    ...
    def test_mlp_pynative(self):
        ...
        cmd = f"msrun --worker_num={device_num} "+\
                    f"--local_worker_num={device_num} "+\
                    f"--master_port=8118 "+\
                    f"--log_dir=msrun_log_pynative_mlp_has_gate "+\
                    f"--join=True "+\
                    f"--cluster_time_out=300 "+\
                    f"{scripts_cmd}"
        ret = os.system(cmd)
    ...
    def test_pynative_with_golden_loss(self):
        ...
        Expectation: relative error smaller than 1e-3
        """
        golden_log_path = f'msrun_log_graph_mlp_has_gate/worker_0.log'
        pynative_log_path = f'msrun_log_pynative_mlp_has_gate/worker_0.log'
