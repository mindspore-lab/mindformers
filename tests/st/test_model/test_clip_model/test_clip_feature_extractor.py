...
How to run this:
windows:  pytest .\\tests\\st\\test_model\\test_clip_model\\test_clip_feature_extractor.py
linux:  pytest ./tests/st/test_model/test_clip_model/test_clip_feature_extractor.py
"""
import os
import numpy as np
...
def test_auto_feature_extractor():
    ...
    fe_c = ClipFeatureExtractor.from_pretrained("clip_vit_b_32")
    fe_d = ClipFeatureExtractor.from_pretrained(yaml_path)
    fe_e = ClipFeatureExtractor(image_feature_extractor=img_fe)
    ...
    assert isinstance(fe_d, ClipFeatureExtractor)
    assert isinstance(fe_e, ClipFeatureExtractor)
