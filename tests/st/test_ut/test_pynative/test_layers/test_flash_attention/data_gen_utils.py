...
def get_init_params(config: TransformerConfig, seq_length=8, batch_size=2):
    ...
    return {
        "query": ms.tensor(0.01 * np.random.randn(*shape), ms.float32),
        "key": ms.tensor(0.01 * np.random.randn(*shape), ms.float32),
        "value": ms.tensor(0.01 * np.random.randn(*shape), ms.float32),
