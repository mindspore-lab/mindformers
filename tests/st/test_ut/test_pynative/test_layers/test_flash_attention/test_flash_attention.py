...
class TestFlashAttention:
    ...
    def setup_method(self):
        ...
        self.config = TransformerConfig(multi_latent_attention=False,
                                           hidden_size=4,
    ...
    def run_test(self, soft_max_scale=None, accuracy=True):
        """Helper function to run test and check results"""
        self.flash_attention = FlashAttention(config=self.config, layer_number=1, softmax_scale=soft_max_scale)
        ...
        if accuracy:
            ...
            assert DoubleBenchmarkComparator.check_pass_or_not(npu_output, gpu_output, golden_output), (
                f"FlashAttention soft_max_scale={soft_max_scale} test failed.\n"
                f"NPU output:\n{npu_output}\n\n"
                f"GPU output:\n{gpu_output}\n\n"
                f"Golden output:\n{golden_output}"
            )
