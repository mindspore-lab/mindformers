...
from mindformers import MindFormerConfig, CausalLanguageModelDataset
...
class TestCausalLanguageModelDataset(unittest.TestCase):
    ...
    def test_dynamic_batch(self):
        ...
        for item in dataset:
            ...
            assert len(item) == 1
            break
        func = dyn_batch_wrapper(1, 1, -100)
        res = func([[-100], [2, 3, 4]], [])
        assert all(res[0][0] == [-100, -100, -100])
        assert res[0][1] == [2, 3, 4]
        res = func([[2, 3, 4], [-100]], [])
        assert res[0][0] == [2, 3, 4]
        assert all(res[0][1] == [-100, -100, -100])
        func = dyn_batch_wrapper(None, None)
        res = func([[2, 3, 4], [-100]])
        assert res == ()
    ...
    def test_process_raw_text_data(self):
        ...
        get_cifar100_data(self.path)
        config = copy.deepcopy(self.default_config)
        config.data_loader.type = "Cifar100DataLoader"
        config.data_loader.dataset_dir = self.path
        config.input_columns = ["image", "text", "label"]
        dataset = CausalLanguageModelDataset(config)
        for item in dataset:
            assert len(item) == 3
