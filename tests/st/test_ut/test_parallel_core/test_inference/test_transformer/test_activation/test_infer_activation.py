...
SINGLE_CARD_TEST_CASES = [
    (
        # 并行策略: 单卡, batch_size: 2, seq_length: 2, hidden_size: 32
        # expected result: 功能跑通, 精度对齐。
        {"batch_size": 2, "seq_length": 2, "hidden_size": 32},
        {"output": "output"},
        False
    )
]
...
class TestInferSiLU:
    """Test class for SiLU with different configurations"""
    OUTPUT_MS_FILENAME = "output_ms_silu.npz"
    ...
    def setup_method(self):
        ...
        self.run_script_path = self.sh_path / "run_infer_activation.py"
    ...
    def check_function(output_ms_dict, model_args, data_keys):
        ...
        for key, _ in data_keys.items():
            ...
            input_data = params["multiplicand_input"]
