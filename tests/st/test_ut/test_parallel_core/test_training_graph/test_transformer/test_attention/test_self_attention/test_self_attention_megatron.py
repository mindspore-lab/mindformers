...
SINGLE_CARD_TEST_CASES = [
    ({"use_flash_attn": True, "num_query_groups": 4, "q_layernorm": None, "k_layernorm": None},
     {"output": "output_query_group_4", "bias": "bias_query_group_4"}, False),
    ({"use_flash_attn": True, "num_query_groups": 8, "q_layernorm": None, "k_layernorm": None},
     {"output": "output_query_group_8", "bias": "bias_query_group_8"}, False),
...
FOUR_CARD_TEST_CASES = [
    ({"use_flash_attn": True, "num_query_groups": 4, "q_layernorm": None, "k_layernorm": None},
     {"output": "output_query_group_4", "bias": "bias_query_group_4"}, False, 2),
    ({"use_flash_attn": True, "num_query_groups": 8, "q_layernorm": None, "k_layernorm": None},
     {"output": "output_query_group_8", "bias": "bias_query_group_8"}, False, 2),
    ({"use_flash_attn": True, "num_query_groups": 4, "q_layernorm": "Norm", "k_layernorm": "Norm"},
     {"output": "output_query_group_4", "bias": "bias_query_group_4"}, False, 2),
]
...
def build_msrun_command_list(
    ...
    if worker_num == 1:
        ...
        cmd_list = [
            "msrun", f"--worker_num={worker_num}",
            f"--local_worker_num={local_worker_num}", "--master_port=8167", f"--log_dir={log_dir}", "--join=True",]
    cmd_list += [
        str(run_script_path), f"--seq_len={seq_len}", f"--batch_size={batch_size}", f"--hidden_size={hidden_size}",
        f"--kv_hidden_size={kv_hidden_size}", f"--num_attention_heads={num_attention_heads}",
        f"--has_bias={str(has_bias).lower()}", f"--use_flash_attn={str(use_flash_attn).lower()}",
        f"--num_query_groups={num_query_groups}", f"--q_layernorm={q_layernorm if q_layernorm else 'None'}",
        f"--k_layernorm={k_layernorm if k_layernorm else 'None'}", f"--output_path={output_path_param}",
...
class TestSelfAttentionMegatron:
    ...
    def check_acc(self, output_ms_dict, data_keys):
        ...
        for key, data_key in data_keys.items():
            ...
            DoubleBenchmarkComparator.check_pass_or_not(
                npu_data=npu_data, gpu_data=gpu_data,
                golden_data=golden_data, standard=standard)

    def run_test(self, worker_num, local_worker_num, model_args,
                 data_keys, tmp_path, tensor_parallel=1, expect_error=False,):
        ...
        cmd_list = build_msrun_command_list(worker_num=worker_num, local_worker_num=local_worker_num,
                                            q_layernorm=model_args["q_layernorm"], log_dir=log_dir_path,
                                            run_script_path=self.run_script_path, seq_len=SEQ_LEN,
                                            kv_hidden_size=KV_HIDDEN_SIZE, num_attention_heads=NUM_HEAD, has_bias=True,
                                            use_flash_attn=model_args["use_flash_attn"], batch_size=BATCH_SIZE,
                                            num_query_groups=model_args["num_query_groups"],
                                            k_layernorm=model_args["k_layernorm"], hidden_size=HIDDEN_SIZE,
                                            output_path_param=output_file_path, tensor_parallel=tensor_parallel)

        result = subprocess.run(cmd_list, shell=False, capture_output=True, text=True, check=False)

        if expect_error:
            assert result.returncode != 0, (f"Expected an error but test script passed. "
                                            f"Stdout:\n{result.stdout}\nStderr:\n{result.stderr}")
        else:
            assert result.returncode == 0, (f"Test script failed with non-zero exit code: "
                                            f"{result.returncode}.\nStdout:\n{result.stdout}\n"
                                            f"Stderr:\n{result.stderr}")
    ...
    def test_single_card_configurations(self, model_args, data_keys, expect_error, tmp_path):
        ...
        self.run_test(
            worker_num=1, local_worker_num=1, model_args=model_args, data_keys=data_keys,
    ...
    def test_four_cards_configurations(self, model_args, data_keys, expect_error, tensor_parallel, tmp_path):
        ...
        self.run_test(
            worker_num=4, local_worker_num=4, model_args=model_args, data_keys=data_keys,
            expect_error=expect_error, tmp_path=tmp_path, tensor_parallel=tensor_parallel)
