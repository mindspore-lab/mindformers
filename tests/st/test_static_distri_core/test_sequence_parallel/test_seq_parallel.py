...
base_command = ('msrun --worker_num={device_num} --local_worker_num={device_num} '
                ...
                'run_seq_parallel.py --dp {dp} --cp {cp} --tp {tp}')
...
class TestSequenceParallel:
    """A test class for testing sequence parallel"""
    ...
    def test_parallel_transformer_layer_on_single(self):
        """
        Feature: sequence parallel
        Description: Test sequence parallel
        Exception: AssertionError
        """
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        dp, cp, tp = (1, 1, 1)
        ret = os.system(f"python {sh_path}/run_seq_parallel.py --dp {dp} --cp {cp} --tp {tp}")
