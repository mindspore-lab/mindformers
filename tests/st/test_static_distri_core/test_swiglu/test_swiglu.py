...
base_command = ('msrun --worker_num={device_num} --local_worker_num={device_num} '
                ...
                'run_swiglu.py --dp {dp} --cp {cp} --tp {tp}')
...
class TestSwiGlu:
    ...
    def test_swiglu_on_single(self):
        """
        Feature: SwiGlu
        Description: Test SwiGlu
        Exception: AssertionError
        """
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        dp, cp, tp = (1, 1, 1)
        ret = os.system(f"python {sh_path}/run_swiglu.py --dp {dp} --cp {cp} --tp {tp}")
    ...
    def test_swiglu_on_parallel(self):
        """
        Feature: SwiGlu
        Description: Test SwiGlu
        Exception: AssertionError
