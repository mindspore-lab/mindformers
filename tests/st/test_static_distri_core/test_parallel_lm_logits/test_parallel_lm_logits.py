...
base_command = ('msrun --worker_num={device_num} --local_worker_num={device_num} '
                ...
                'run_parallel_lm_logits.py --dp {dp} --cp {cp} --tp {tp}')
...
class TestParallelLMLogits:
    """A test class for testing ParallelLMLogits"""

    @pytest.mark.level1
    @pytest.mark.platform_arm_ascend910b_training
    @pytest.mark.env_onecard
    def test_vocab_parallel_embedding_on_single(self):
        """
        Feature: ParallelLMLogits
        Description: Test ParallelLMLogits
        Exception: AssertionError
        """
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        dp, cp, tp = (1, 1, 1)
        ret = os.system(
            f"python {sh_path}/run_vocab_parallel_embedding.py --dp {dp} --cp {cp} --tp {tp} --has_bias"
        )
    ...
    def test_vocab_parallel_embedding_on_parallel(self):
        """
        Feature: ParallelLMLogits
        Description: Test ParallelLMLogits
        Exception: AssertionError
