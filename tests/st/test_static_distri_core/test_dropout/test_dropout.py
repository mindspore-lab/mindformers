...
base_command = ('msrun --worker_num={device_num} --local_worker_num={device_num} '
                ...
                'run_dropout.py --dp {dp} --cp {cp} --tp {tp}')
...
class TestGELU:
    """A test class for testing GELU"""

    @pytest.mark.level1
    @pytest.mark.platform_arm_ascend910b_training
    @pytest.mark.env_onecard
    def test_gelu_on_single(self):
        """
        Feature: GELU
        ...
        dp, cp, tp = (1, 1, 1)
        ret = os.system(f"python {sh_path}/run_dropout.py --dp {dp} --cp {cp} --tp {tp}")
    ...
    def test_gelu_on_parallel(self):
        """
        Feature: GELU
        Description: Test GELU
        Exception: AssertionError
