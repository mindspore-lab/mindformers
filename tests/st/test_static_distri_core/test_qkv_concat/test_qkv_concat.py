...
# ============================================================================
"""test parallel transformer layer"""
import os
import pytest
...
base_command = ('msrun --worker_num={device_num} --local_worker_num={device_num} '
                ...
                'run_qkv_concat.py --dp {dp} --cp {cp} --tp {tp}')
...
class TestQKVConcat:
    """A test class for testing qkv concat weight format"""
    ...
    def test_parallel_transformer_layer_on_single(self):
        """
        Feature: qkv concat as megatron
        Description: Test qkv concat as megatron single
        Exception: AssertionError
        """
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        dp, cp, tp = (1, 1, 1)
        ret = os.system(f"python {sh_path}/run_qkv_concat.py --dp {dp} --cp {cp} --tp {tp}")
    ...
    def test_parallel_transformer_layer_on_parallel(self):
        """
        Feature: qkv concat as megatron
        Description: Test qkv concat as megatron parallel
        Exception: AssertionError
        """
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        device_num = 4
