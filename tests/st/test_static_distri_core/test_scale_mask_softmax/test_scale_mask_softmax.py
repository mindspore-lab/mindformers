...
base_command = ('msrun --worker_num={device_num} --local_worker_num={device_num} '
                ...
                'run_softmax.py --dp {dp} --cp {cp} --tp {tp}')
...
class TestScaleMaskSoftmax:
    ...
    def test_scale_mask_softmax_on_single(self):
        """
        Feature: ScaleMaskSoftmax
        Description: Test ScaleMaskSoftmax
        Exception: AssertionError
        """
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        dp, cp, tp = (1, 1, 1)
        ret = os.system(f"python {sh_path}/run_softmax.py --dp {dp} --cp {cp} --tp {tp}")
    ...
    def test_scale_mask_softmax_on_parallel(self):
        """
        Feature: ScaleMaskSoftmax
        Description: Test ScaleMaskSoftmax
        Exception: AssertionError
