...
base_command = ('msrun --worker_num={device_num} --local_worker_num={device_num} '
                ...
                'run_lora_row.py --dp {dp} --cp {cp} --tp {tp}')
...
class TestLoRARowParallelLinear:
    """A test class for testing LoRARowParallelLinear"""

    @pytest.mark.level1
    @pytest.mark.platform_arm_ascend910b_training
    @pytest.mark.env_onecard
    def test_lora_row_parallel_linear_on_single(self):
        """
        Feature: LoRARowParallelLinear
        Description: Test LoRARowParallelLinear
        Exception: AssertionError
        """
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        dp, cp, tp = (1, 1, 1)
        has_bias = True
        ret = os.system(f"python {sh_path}/run_lora_row.py --dp {dp} --cp {cp} --tp {tp} --has_bias {has_bias}")
    ...
    def test_lora_row_parallel_linear_on_parallel(self):
        """
        Feature: LoRARowParallelLinear
        Description: Test LoRARowParallelLinear
        Exception: AssertionError
