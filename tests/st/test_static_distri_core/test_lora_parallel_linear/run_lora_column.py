...
# ============================================================================
"""Run lora column parallel linear test"""
import argparse
import os
import numpy as np
...
from mindformers.experimental.graph.tensor_parallel import LoRAColumnParallelLinear
...
class LoRAColumnNet(nn.Cell):
    """LoraColumnNet for test lora column parallel linear"""

    def __init__(self, input_size, output_size, config: TransformerConfig, has_bias=True):
        super(LoRAColumnNet, self).__init__()

        self.linear = LoRAColumnParallelLinear(input_size=input_size, output_size=output_size,
    ...
    def construct(self, input_: Tensor, weight: Tensor = None):
        output_, output_bias = self.linear(input_, weight)
...
net = LoRAColumnNet(hidden_size, output_size_, config_, has_bias_)
