...
class TestParallelMLP:
    """A test class for testing ParallelMLP"""

    @pytest.mark.level0
    @pytest.mark.platform_arm_ascend910b_training
    @pytest.mark.env_onecard
    def test_parallel_mlp_on_single(self):
        """
        Feature: ParallelMLP
        Description: Test ParallelMLP
        Exception: AssertionError
        """
        sh_path = os.path.split(os.path.realpath(__file__))[0]
        dp, cp, tp = (1, 1, 1)
        ret = os.system(f"python {sh_path}/run_mlp.py --dp {dp} --cp {cp} --tp {tp}")
    ...
    def test_parallel_mlp_on_parallel(self):
        """
        Feature: ParallelMLP
        Description: Test ParallelMLP
        Exception: AssertionError
