...
from xformer.tools.utils import get_num_nodes_devices, get_rank_info, const, \
    convert_nodes_devices_input, generate_rank_list, check_in_modelarts, check_list
...
def get_logger(logger_name: str = 'run_xformer', **kwargs) -> logging.Logger:
    ...
    if check_in_modelarts():
        file_save_dir = const.MODELARTS_LOG_FILE_DIR
        const.RANK_DIR_FORMATTER = os.path.join(const.RANK_DIR_FORMATTER, 'log')
    if append_rank_dir:
        rank_str = const.RANK_DIR_FORMATTER.format(rank_id)
        file_save_dir = os.path.join(file_save_dir, rank_str)
    ...
    logger.setLevel(_convert_level('INFO'))
