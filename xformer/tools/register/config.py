...
class XFormerConfig(dict):
    ...
    Example:
        test.yaml:
            a:1
        >>> cfg = XFormerConfig('./test.yaml')
        >>> cfg.a
        1

        >>> cfg = XFormerConfig(**dict(a=1, b=dict(c=[0,1])))
        >>> cfg.b
        {'c': [0, 1]}
    """

    def __init__(self, *args, **kwargs):
        super(XFormerConfig, self).__init__()
        cfg_dict = {}
        ...
        for arg in args:
            if isinstance(arg, str):
                if arg.endswith('yaml') or arg.endswith('yml'):
                    raw_dict = XFormerConfig._file2dict(arg)
        ...
        XFormerConfig._dict2config(self, cfg_dict)
    ...
    def merge_from_dict(self, options):
        ...
        Examples:
            ...
            >>> cfg = XFormerConfig(**dict(model=dict(backbone=dict(type='vit'))))
        ...
        for full_key, value in options.items():
            ...
            for sub_key in key_list[:-1]:
                d.setdefault(sub_key, XFormerConfig())
                d = d[sub_key]
            sub_key = key_list[-1]
            d[sub_key] = value
        merge_dict = XFormerConfig._merge_a_into_b(option_cfg_dict, self)
        XFormerConfig._dict2config(self, merge_dict)
    ...
    def _merge_a_into_b(a, b):
        ...
        for k, v in a.items():
            if isinstance(v, dict) and k in b:
                b[k] = XFormerConfig._merge_a_into_b(v, b[k])
            else:
                b[k] = v
        return b
    ...
    def _file2dict(filename=None):
        ...
        if BASE_CONFIG in cfg_dict:
            ...
            for base_filename in base_filenames:
                cfg_dict_item = XFormerConfig._file2dict(
            ...
            # Merge config
            base_cfg_dict = XFormerConfig._merge_a_into_b(cfg_dict, base_cfg_dict)
        ...
        if isinstance(dic, dict):
            for key, value in dic.items():
                if isinstance(value, dict):
                    sub_config = XFormerConfig()
                    dict.__setitem__(config, key, sub_config)
                    XFormerConfig._dict2config(sub_config, value)
                else:
                    config[key] = dic[key]
