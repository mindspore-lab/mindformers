...
from mindspore.nn.transformer import TransformerRecomputeConfig, TransformerOpParallelConfig
...
from mindspore.nn.transformer.op_parallel_config import default_dpmp_config, OpParallelConfig, _Config, _check_config, \
    MoEParallelConfig
...
class RelaPosEmbeddingsGenerator(nn.Cell):
    ...
    def __init__(self,
        ...
        zeros = ops.Zeros()
        self.embeddings_table = zeros((self.vocab_size, depth), mindspore.float16)

        position = mindspore.numpy.arange(0, self.vocab_size)
        position = self.reshape(position, (self.vocab_size, -1))
        position = self.cast(position, mstype.float16)
        div_term = self.exp(self.cast(mindspore.numpy.arange(0, depth, 2),
                                      mstype.float16) * (-math.log(10000.0) / depth))
        position = self.tile(position, (1, div_term.shape[0]))
        div_term = self.reshape(div_term, (-1, div_term.shape[0]))
        div_term = self.tile(div_term, (self.vocab_size, 1))
        self.embeddings_table[:, 0::2] = ops.sin(self.mul(position, div_term))
        self.embeddings_table[:, 1::2] = ops.cos(self.mul(position, div_term))
        self.relative_positions_matrix = RelaPosMatrixGenerator(max_relative_position=max_relative_position)
        self.relative_positions_matrix_out = self.relative_positions_matrix(length)

        flat_relative_positions_matrix = self.reshape(self.relative_positions_matrix_out, (-1,))
        one_hot_relative_positions_matrix = self.one_hot(flat_relative_positions_matrix)
        one_hot_relative_positions_matrix = self.cast(one_hot_relative_positions_matrix, mstype.float16)
        self.embeddings = self.matmul(one_hot_relative_positions_matrix, self.embeddings_table)
        my_shape = self.shape(self.relative_positions_matrix_out) + (self.depth,)
        self.embeddings = self.reshape(self.embeddings, my_shape)
...
class MultiHeadAttention(Cell):
    ...
    def __init__(self, batch_size,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation():
            ...
            self.scores_mul = Tensor(math.sqrt(self.size_per_head))
        ...
        if self.use_relative_positions:
            ...
            self._generate_relative_positions_embeddings = \
                RelaPosEmbeddingsGenerator(depth=self.size_per_head,
                                           max_relative_position=64,
                                           length=self.src_seq_length)

        self.cast_compute_type = SaturateCast(dst_type=compute_dtype)
    ...
    def construct(self, query_tensor, key_tensor, value_tensor, attention_mask, key_past=None,
        ...
        # print("hidden_states:", query_tensor)
        query = self.dense1(query_tensor)
        # print("hidden", query_tensor)
    ...
    def _attn(self, query, key, value, attention_mask):
        ...
        factor_rela = P.Cast()(self.scores_mul, P.DType()(query))
        query_mod = self.real_div(query, factor)
        key_mod = self.real_div(key, factor)
        score = self.batch_matmul(query_mod, key_mod)

        ori_dtype = P.DType()(score)
        score = P.Cast()(score, self.softmax_dtype)

        if self.use_relative_positions:
            trans_shape_relative = (2, 0, 1, 3)
            trans_shape_position = (1, 2, 0, 3)
            ...
            query_layer_t = self.transpose(query, trans_shape_relative)
            ...
            key_position_scores_r_t = self.real_div(key_position_scores_r_t, factor_rela)

            score = score + key_position_scores_r_t
        ...
        if self.use_relative_positions:
            ...
            attention_probs_t = self.transpose(attention_probs, trans_shape_relative)
            ...
            value_position_scores_r_t = self.transpose(value_position_scores_r,
                                                       trans_shape_position)
            weighted_values = weighted_values + value_position_scores_r_t
    ...
    def construct(self, hidden_states, attention_mask, init_reset=True, batch_valid_length=None):
        ...
        for i in range(self.num_layers):
            hidden_states, present = self.blocks[i](hidden_states,
                                                    ...
                                                    init_reset,
                                                    batch_valid_length)
            # print("encoder output", hidden_states)
            present_layer = present_layer + (present,)
...
class Transformer(Cell):
    ...
    def __init__(self,
        ...
        # print("Nezha transformer parallel_config", parallel_config)
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,) and _is_sharding_propagation():
            _check_config(parallel_config)
            self.batch_size = batch_size
            self.hidden_size = hidden_size
            ...
            self.add = P.Add().shard(((), ()))
            self.aux_loss = Tensor(0.0, mstype.float32)
            # print("use_relative_positions", use_relative_positions)
            if encoder_layers > 0:
                self.encoder = TransformerEncoder(num_layers=encoder_layers,
                                                  batch_size=batch_size,
                                                  hidden_size=hidden_size,
