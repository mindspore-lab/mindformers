...
def create_wiki_dataset(batch_size, data_path, device_num=1, rank=0, drop=True):
    ...
    dataset = ds.MindDataset(data, columns_list=["input_ids",
                                                 "attention_mask",
                                                 "labels"], shuffle=True, num_shards=device_num, shard_id=rank)
    type_cast_op = C.TypeCast(mstype.int32)
    fp_type_cast_op = C.TypeCast(mstype.float32)
    dataset = dataset.map(input_columns="input_ids", operations=type_cast_op)
    dataset = dataset.map(input_columns="attention_mask", operations=fp_type_cast_op)
